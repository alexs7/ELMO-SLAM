...
pathDatasetKITTI='/media/ziwen/2454B43C54B41312/kitti_data_odometry_gray_label/kitti_data_odometry_gray/sequences'
...
#https://unix.stackexchange.com/questions/12068/how-to-measure-time-of-program-execution-and-store-that-inside-a-variable
    TIME_START=$(date +%s.%N)
    ./Examples/Monocular/mono_kitti Vocabulary/ORBvoc.txt Examples/Monocular/"$Vocabulary_file" "$pathDatasetKITTI"/"$j" ./"$DIRECOTRY"/sh_elmo_kitti_mono_"$j".txt
    TIME_END=$(date +%s.%N)
