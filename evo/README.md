...
* `--align_origin` aligns trajectories when their initial poses are different - this is important for KITTI-360 dataset because the initial pose in KITTI-360 ground truth is not at `(0, 0 ,0)`. 
...
* `--correct_scale` tries to match the scales of different trajectories - this is needed for all monocular experiments


3. Measure particular metrics - Absolute Pose Error
```
...
4. Measure particular metrics - Relative Pose Error
```
