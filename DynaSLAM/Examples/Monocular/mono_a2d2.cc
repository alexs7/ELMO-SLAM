...
int main(int argc, char **argv)
{
    if(argc != 5)
    {
        cerr << endl << "Usage: ./mono_a2d2 path_to_vocabulary path_to_settings path_to_sequence path_to_label" << endl;
        return 1;
    }
...
{
    vector<cv::String> fn;
    cv::glob((strPathToSequence+"/*.png"), fn, false);
...
//        cout<<vstrImageFilenames[i]<<endl;
    ...
    vector<cv::String> tn;
    cv::glob((strPathToSequence+"/*.json"), tn, false);
